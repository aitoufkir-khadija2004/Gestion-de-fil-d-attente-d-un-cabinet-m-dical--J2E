...
<body class="bg-light">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    ...
                    Liste des Secrétaires
                    <span class="badge bg-light text-dark ms-2"><%= secretaires != null ? secretaires.size() : 0 %></span>
                </h5>
            ...
            <div class="card-body">
                <% if (secretaires != null && secretaires.size() > 0) { %>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr <%= !secretaire.isActif() ? "class='table-secondary'" : "" %>>
                                    <td><strong><%= secretaire.getCodeUser() %></strong></td>
                                    <td><%= secretaire.getNom() != null ? secretaire.getNom() : "-" %></td>
                                    <td><%= secretaire.getPrenom() != null ? secretaire.getPrenom() : "-" %></td>
                                    <td>
                                        <% if (secretaire.getEmail() != null && !secretaire.getEmail().isEmpty()) { %>
                                    ...
                                    <td>
                                        <% if (secretaire.getTelephone() != null && !secretaire.getTelephone().isEmpty()) { %>
                                    ...
                                    <td>
                                        <% if (secretaire.getService() != null && !secretaire.getService().isEmpty()) { %>
                                            <span class="badge bg-warning text-dark">
                                                <i class="bi bi-building me-1"></i>
                                                <%= secretaire.getService() %>
                                            </span>
                                        <% } else { %>
                                            <span class="text-muted">-</span>
                                        <% } %>
                                    ...
                                    <td>
                                        ...
                                        <button type="button" class="btn btn-warning btn-sm me-1" 
                                                ...
                                                data-bs-target="#modalModifierSecretaire<%= secretaire.getId() %>">
                                        ...
                                        <% if (secretaire.isActif()) { %>
                                            <button type="button" class="btn btn-secondary btn-sm" 
                                                    onclick="confirmerDesactivation(<%= secretaire.getId() %>, '<%= secretaire.getPrenom() %> <%= secretaire.getNom() %>')">
                                                <i class="bi bi-x-circle-fill"></i>
                                            </button>
                                        <% } else { %>
                                            <button type="button" class="btn btn-success btn-sm" 
                                                    onclick="confirmerActivation(<%= secretaire.getId() %>, '<%= secretaire.getPrenom() %> <%= secretaire.getNom() %>')">
                                ...
                                <div class="modal fade" id="modalModifierSecretaire<%= secretaire.getId() %>" tabindex="-1">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header bg-warning">
                                                <h5 class="modal-title">
                                                    <i class="bi bi-pencil me-2"></i>
                                                    Modifier Secrétaire - <%= secretaire.getCodeUser() %>
                                                </h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <form action="${pageContext.request.contextPath}/admin/modifier-utilisateur" method="post">
                                                <div class="modal-body">
                                                    <input type="hidden" name="userId" value="<%= secretaire.getId() %>">
                                                    <input type="hidden" name="role" value="SECRETAIRE">
                                                    <input type="hidden" name="redirectPage" value="secretaires">
                                                    
                                                    <div class="row">
                                                        <div class="col-md-6 mb-3">
                                                            <label class="form-label">Nom <span class="text-danger">*</span></label>
                                                            <input type="text" class="form-control" name="nom" 
                                                                   value="<%= secretaire.getNom() %>" required>
                                                        </div>
                                                        <div class="col-md-6 mb-3">
                                                            <label class="form-label">Prénom <span class="text-danger">*</span></label>
                                                            <input type="text" class="form-control" name="prenom" 
                                                                   value="<%= secretaire.getPrenom() %>" required>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-6 mb-3">
                                                            ...
                                                            <input type="email" class="form-control" name="email" 
                                                                   value="<%= secretaire.getEmail() %>" required>
                                                        </div>
                                                        <div class="col-md-6 mb-3">
                                                            <label class="form-label">Téléphone</label>
                                                            <input type="tel" class="form-control" name="telephone" 
                                                                   value="<%= secretaire.getTelephone() != null ? secretaire.getTelephone() : "" %>">
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" class="form-control" name="service" 
                                                               value="<%= secretaire.getService() != null ? secretaire.getService() : "" %>"
                ...
                <% } else { %>
                    <div class="text-center py-5">
                        ...
                        <h5 class="mt-3">Aucune secrétaire</h5>
                        <p class="text-muted">Aucune secrétaire enregistrée dans le système.</p>
                        <a href="${pageContext.request.contextPath}/admin/creer-utilisateur" 
                           class="btn btn-info">
                            <i class="bi bi-plus-circle me-1"></i>
    ...
    <script>
        function confirmerActivation(userId, nom) {
            if (confirm(' Voulez-vous activer la secrétaire "' + nom + '" ?')) {
        ...
        function confirmerDesactivation(userId, nom) {
            if (confirm('Voulez-vous désactiver la secrétaire "' + nom + '" ?')) {
